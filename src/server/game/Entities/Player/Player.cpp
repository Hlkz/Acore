    ...
    if (!pItem2)
    {
        ...
        if (IsAlive())
        {
            ...
            if (pProto && IsInCombat() && (pProto->Class == ITEM_CLASS_WEAPON || pProto->InventoryType == INVTYPE_RELIC) && m_weaponChangeTimer == 0)
            {
                ...
                if (!spellProto)
                    TC_LOG_ERROR("entities.player", "Weapon switch cooldown spell %u couldn't be found in Spells table", cooldownSpell);
                else
                {
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    else if (transGUID)
    {
        ...
        else
        {
            ...
            {
                TC_LOG_ERROR("entities.player", "Player (guidlow %u) have problems with transport guid (%u). Teleport to bind location.",guid, transGUID);
                RelocateToHomebind();
            }
        }
    }
