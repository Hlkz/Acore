...
void KillRewarder::_RewardGroup()
{
    if (_maxLevel)
    {
        ...
        {
            const bool isDungeon = !_isPvP && sDBCMgr->GetMapEntry(_killer->GetMapId())->IsDungeon();
            if (!_isBattleGround)
            {
                // 3.1.2. Alter group rate if group is in raid (not for battlegrounds).
                const bool isRaid = !_isPvP && sDBCMgr->GetMapEntry(_killer->GetMapId())->IsRaid() && _group->isRaidGroup();
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    if (!AccountMgr::IsAdminAccount(GetSession()->GetSecurity()) && DisableMgr::IsDisabledFor(DISABLE_TYPE_MAP, mapid, this))
    {
...
Creature* Player::GetNPCIfCanInteractWith(uint64 guid, uint32 npcflagmask)
{
    ...
    // not unfriendly
    if (FactionTemplateEntry const* factionTemplate = sDBCMgr->GetFactionTemplateEntry(creature->getFaction()))
...
void Player::DeleteFromDB(uint64 playerguid, uint32 accountId, bool updateRealmChars, bool deleteFinally)
{
    ...
    else if (CharacterNameData const* nameData = sWorld->GetCharacterNameData(playerguid))    // To avoid a query, we select loaded data. If it doesn't exist, return.
    {
        if (!nameData)
        {
            TC_LOG_ERROR("entities.player", "Cannot find CharacterNameData entry for player %u from account %u. Could not delete character.", GUID_LOPART(playerguid), accountId);
            return;
        }
        charDelete_method = CHAR_DELETE_REMOVE;
    }
...
void Player::KillPlayer()
{
    ...
    ApplyModFlag(PLAYER_FIELD_BYTES, PLAYER_FIELD_BYTE_RELEASE_TIMER, !sDBCMgr->GetMapEntry(GetMapId())->Instanceable() && !HasAuraType(SPELL_AURA_PREVENT_RESURRECTION));
...
{
    if (UpdateSkill(SKILL_DEFENSE, 1 /*sWorld->getIntConfig(CONFIG_SKILL_GAIN_DEFENSE)*/))
...
void Player::UpdateWeaponSkill(WeaponAttackType attType)
{
    ...
    uint32 weapon_skill_gain = 1; // sWorld->getIntConfig(CONFIG_SKILL_GAIN_WEAPON);
...
void Player::CheckAreaExploreAndOutdoor()
{
    ...
    if (!(currFields & val))
    {
        ...
        if (!areaEntry)
        {
            TC_LOG_ERROR("entities.player", "Player %u discovered unknown area (x: %f y: %f z: %f map: %u areaflag: %u)", GetGUIDLow(), GetPositionX(), GetPositionY(), GetPositionZ(), GetMapId(), areaFlag);
            return;
        }
        ...
        if (areaEntry->area_level > 0)
        {
            ...
            else
            {
                ...
                else if (diff > 5)
                {
                    ...
                    XP = uint32(sObjectMgr->GetBaseXP(areaEntry->area_level)*exploration_percent/100);
                }
                else
                {
...
bool Player::LoadFromDB(uint32 guid, SQLQueryHolder *holder)
{
    ...
    if (ObjectMgr::CheckPlayerName(m_name) != CHAR_NAME_SUCCESS ||
        (!AccountMgr::IsAdminAccount(GetSession()->GetSecurity()) &&
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    mapEntry = sDBCMgr->GetMapEntry(mapId);
    // client without expansion support
    if (mapEntry)
    {
    ...
    // GM state
    if (AccountMgr::IsAnimAccount(GetSession()->GetSecurity()))
    {
...
void Player::LoadCorpse()
{
    ...
    else
    {
        if (Corpse* corpse = GetCorpse())
            ApplyModFlag(PLAYER_FIELD_BYTES, PLAYER_FIELD_BYTE_RELEASE_TIMER, corpse && !sDBCMgr->GetMapEntry(corpse->GetMapId())->Instanceable());
        else
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!IsGameMaster() && ar)
    {
        ...
        MapEntry const* mapEntry = sDBCMgr->GetMapEntry(target_map);
        if (!mapEntry)
            return false;
...
bool Player::_LoadHomeBind(PreparedQueryResult result)
{
    ...
    if (result)
    {
        ...
        MapEntry const* bindMapEntry = sDBCMgr->GetMapEntry(m_homebindMapId);
...
void Player::SaveToDB(bool create /*=false*/)
{
    ...
    if (create)
    {
        ...
        stmt->setUInt32(index++, 0); // m_resetTalentsCost, unused
...
void Player::ResetInstances(uint8 method, bool isRaid)
{
    ...
    for (BoundInstancesMap::iterator itr = m_boundInstances[diff].begin(); itr != m_boundInstances[diff].end();)
    {
        ...
        const MapEntry* entry = sDBCMgr->GetMapEntry(itr->first);
...
bool Player::CanUseBattlegroundObject(GameObject* gameobject)
{
    ...
    if (gameobject)
    {
        ...
        FactionTemplateEntry const* faction = sDBCMgr->GetFactionTemplateEntry(gameobject->GetUInt32Value(GAMEOBJECT_FACTION));
...
void Player::ActivateSpec(uint8 spec)
{
    ...
    for (TalentContainer::const_iterator itr = sDBCMgr->TalentStore.begin(); itr != sDBCMgr->TalentStore.end(); ++itr)
    {
        ...
        for (int8 rank = MAX_TALENT_RANK-1; rank >= 0; --rank)
        {
            ...
            if (const SpellInfo* _spellInfo = sSpellMgr->GetSpellInfo(talentInfo->RankID[rank]))
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)                  // search through the SpellInfo for valid trigger spells
                    if (_spellInfo->Effects[i].TriggerSpell > 0 && _spellInfo->Effects[i].Effect == SPELL_EFFECT_LEARN_SPELL)
                        RemoveSpell(_spellInfo->Effects[i].TriggerSpell, true); // and remove any spells that the talent teaches
            // if this talent rank can be found in the PlayerTalentMap, mark the talent as removed so it gets deleted
            //PlayerTalentMap::iterator plrTalent = m_talents[m_activeSpec]->find(talentInfo->RankID[rank]);
            //if (plrTalent != m_talents[m_activeSpec]->end())
            //    plrTalent->second->state = PLAYERSPELL_REMOVED;
...
void Player::SetSpectator(bool bSpectator)
{
    if (bSpectator)
    {
        if (IsSpectator())
        {
            TC_LOG_ERROR("bg.arena", "Player::SetSpectator: trying to set spectator state for player (GUID: %u) but he already has this state.", GetGUIDLow());
            return;
        }
