...
{
    m_attackTimer[attType] = uint32(GetAttackTime(attType) * (m_modAttackSpeedPct[attType] + (GetTypeId() == TYPEID_PLAYER ? ToPlayer()->GetAttackSpeedPctFromStats() : 0.0f)));
}
...
uint32 Unit::CalcArmorReducedDamage(Unit* victim, const uint32 damage, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        float armorPen = bonusPct + ToPlayer()->GetRatingBonusValue(CR_ARMOR_PENETRATION);

        // Got the value, apply it
        armor -= 10.0f * armorPen;
    }

    if (armor < 0.0f)
        armor = 0.0f;

    return std::max<uint32>(damage - damage * (armor / (armor + 1000.0f)), 1);
}

float Unit::CalcSpellResistance(Unit* victim, SpellSchoolMask schoolMask, SpellInfo const* spellInfo) const
{
    ...
    return 100.0f * float(victimResistance) / float(victimResistance + 100);
}
...
void Unit::CalcAbsorbResist(Unit* victim, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, uint32* resist, SpellInfo const* spellInfo /*= NULL*/)
{
    ...
    float spellResistance = CalcSpellResistance(victim, schoolMask, spellInfo);
        ...
        switch (auraSpellInfo->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_SHAMAN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    default:
                    {
                        // Lightning Shield (overwrite non existing triggered spell call in `spells` table
...
int32 Unit::SpellBaseDamageBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        DoneAdvertisedBenefit += ToPlayer()->GetStat(STAT_INTELLECT) * ToPlayer()->GetStatRatio(SPELLPOWER_PER_INTELLECT);
        DoneAdvertisedBenefit += ToPlayer()->GetStat(STAT_SPIRIT) * ToPlayer()->GetStatRatio(SPELLPOWER_PER_SPIRIT);
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask) const
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        advertisedBenefit += ToPlayer()->GetStat(STAT_INTELLECT) * ToPlayer()->GetStatRatio(SPELLPOWER_PER_INTELLECT);
        advertisedBenefit += ToPlayer()->GetStat(STAT_SPIRIT) * ToPlayer()->GetStatRatio(SPELLPOWER_PER_SPIRIT);
...
{
    float f_BaseAttackTime = GetFloatValue(UNIT_FIELD_BASEATTACKTIME+attType) / (m_modAttackSpeedPct[attType] + (GetTypeId() == TYPEID_PLAYER ? ToPlayer()->GetAttackSpeedPctFromStats() : 0.0f));
...
void Unit::SetAttackTime(WeaponAttackType attType, uint32 val)
{
    ...
    SetFloatValue(UNIT_FIELD_BASEATTACKTIME+attType, uint32(val / (m_modAttackSpeedPct[attType] + (GetTypeId()==TYPEID_PLAYER ? ToPlayer()->GetAttackSpeedPctFromStats() : 0.0f))));
}

void Unit::ApplyAttackTimePercentMod(WeaponAttackType attType, float val, bool apply)
{
    float pctFromStats = GetTypeId() == TYPEID_PLAYER ? ToPlayer()->GetAttackSpeedPctFromStats() : 0.0f;
...
void Unit::Kill(Unit* victim)
{
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        TC_LOG_DEBUG("entities.unit", "We are dead, losing 1 durability");
...
bool Unit::HandleSpellClick(Unit* clicker, int8 seatId)
{
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        ...
        {
            if (IsInMap(caster))
                caster->CastSpell(target, spellInfo, GetVehicleKit() ? TRIGGERED_IGNORE_CASTER_MOUNTED_OR_ON_VEHICLE : TRIGGERED_NONE, NULL, NULL, origCasterGUID);
            else
