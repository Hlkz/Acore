...
bool Creature::UpdateEntry(uint32 entry, CreatureData const* data /*= nullptr*/)
{
    ...
    if (FactionTemplateEntry const* factionTemplate = sDBCMgr->GetFactionTemplateEntry(cInfo->faction))
    {
...
{
    return sWorld->getRate(RATE_CREATURE_HP);
}
...
{
    return sWorld->getRate(RATE_CREATURE_DAMAGE);
}

float Creature::GetSpellDamageMod(int32 Rank) const
{
    return sWorld->getRate(RATE_CREATURE_SPELLDAMAGE);
}
...
void Creature::SendZoneUnderAttackMessage(Player* attacker)
{
    ...
    sWorld->SendZoneMessage(GetZoneId(), &data);
