...
{
    FactionEntry const* a_factionEntry = sDBCMgr->GetFactionEntry(a_faction_id);
    FactionEntry const* h_factionEntry = sDBCMgr->GetFactionEntry(h_faction_id);
    ...
    for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
        ...
        if (Player* player = _GetPlayerForTeam(TeamID, itr, "RewardReputationToTeam"))
        {
            ...
            AddPct(repGain, player->GetTotalAuraModifierByMiscValue(SPELL_AURA_MOD_FACTION_REPUTATION_GAIN, player->GetTeam(true) == ALLIANCE ? a_faction_id : h_faction_id));
            player->GetReputationMgr().ModifyReputation(player->GetTeam(true) == ALLIANCE ? a_factionEntry : h_factionEntry, repGain);
        }
    }
}
...
{
    if (!m_InBGFreeSlotQueue && isBattleground() && !isRated())
    {
