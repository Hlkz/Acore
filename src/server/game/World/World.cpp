...
void World::AddSession_(WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && !AccountMgr::IsGMAccount(s->GetSecurity()) && !HasRecentlyDisconnected(s))
    {
        AddQueuedPlayer(s);
...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfigMgr->GetStringDefault("Motd", "Welcome to Aviana Server."));
    ...
    rate_values[RATE_REPAIRCOST]  = sConfigMgr->GetFloatDefault("Rate.RepairCost", 0.0f); // unused
    if (rate_values[RATE_REPAIRCOST] < 0.0f)
    {
        TC_LOG_ERROR("server.loading", "Rate.RepairCost (%f) must be >=0. Using 0.0 instead.", rate_values[RATE_REPAIRCOST]);
        rate_values[RATE_REPAIRCOST] = 0.0f;
    }
    rate_values[RATE_CREATURE_HP]                     = sConfigMgr->GetFloatDefault("Rate.Creature.HP", 1.0f);
    rate_values[RATE_CREATURE_DAMAGE]                 = sConfigMgr->GetFloatDefault("Rate.Creature.Damage", 1.0f);
    rate_values[RATE_CREATURE_SPELLDAMAGE]            = sConfigMgr->GetFloatDefault("Rate.Creature.SpellDamage", 1.0f);
    ...
    m_bool_configs[CONFIG_ACTIVE_WORLD_OBJECTS] = sConfigMgr->GetBoolDefault("ActiveWorldObjects", true); // hack tofix?
    ...
    m_bool_configs[CONFIG_ALLOW_TWO_SIDE_INTERACTION_ARENA]   = sConfigMgr->GetBoolDefault("AllowTwoSide.Interaction.Arena", true);
    m_bool_configs[CONFIG_ALLOW_TWO_SIDE_TRADE]               = sConfigMgr->GetBoolDefault("AllowTwoSide.trade", true);
    m_bool_configs[CONFIG_ALLOW_TWO_SIDE_BATTLEGROUND]        = sConfigMgr->GetBoolDefault("AllowTwoSide.battleground", true);
    ...
    m_int_configs[CONFIG_SKIP_CINEMATICS] = sConfigMgr->GetIntDefault("SkipCinematics", 2);
    if (int32(m_int_configs[CONFIG_SKIP_CINEMATICS]) < 0 || m_int_configs[CONFIG_SKIP_CINEMATICS] > 2)
    {
        TC_LOG_ERROR("server.loading", "SkipCinematics (%i) must be in range 0..2. Set to 2.", m_int_configs[CONFIG_SKIP_CINEMATICS]);
    ...
    m_int_configs[CONFIG_INSTANCE_UNLOAD_DELAY] = sConfigMgr->GetIntDefault("Instance.UnloadDelay", 0);
    ...
    m_int_configs[CONFIG_DEATH_SICKNESS_LEVEL]           = sConfigMgr->GetIntDefault ("Death.SicknessLevel", 256);
    ...
    m_int_configs[CONFIG_BATTLEGROUND_PREMATURE_FINISH_TIMER]        = sConfigMgr->GetIntDefault ("Battleground.PrematureFinishTimer", 0);
    ...
    m_int_configs[CONFIG_ARENA_SEASON_ID]                            = sConfigMgr->GetIntDefault ("Arena.ArenaSeason.ID", 8);
    ...
    m_int_configs[CONFIG_CHARDELETE_METHOD] = sConfigMgr->GetIntDefault("CharDelete.Method", 1);
...
void World::SetInitialWorldSettings()
{
    ...
    if (!MapManager::ExistMapAndVMap(1, -452.84f, -2625.76f))
    {
    ...
    TC_LOG_INFO("server.loading", "Loading Map Template...");
    ...
    TC_LOG_INFO("server.loading", "Loading Transmogrifications...");
    ...
    TC_LOG_INFO("server.loading", "Calculate ranks distrib reset time...");
...
void World::SendGMText(uint32 string_id, ...)
{
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        WorldSession* session = itr->second;
        if (!session || !AccountMgr::IsAnimAccount(session->GetSecurity()))
            continue;
...
{
    TC_LOG_INFO("misc", "Ranks distrib !");
    DistributeRanks();
