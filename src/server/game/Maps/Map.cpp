...
i_mapEntry(sDBCMgr->GetMapEntry(id)), i_spawnMode(SpawnMode), i_InstanceId(InstanceId),
...
{
    return i_mapEntry ? i_mapEntry->Name[sWorld->GetDefaultDbcLocale()].c_str() : "UNNAMEDMAP\x0";
}
...
void Map::CreateMapData()
{
    ...
    TC_LOG_DEBUG("maps", "Map::CreateMapData: %u WMScript Initialized", GetId());
}
