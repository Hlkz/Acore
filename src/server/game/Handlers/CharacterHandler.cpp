...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recvData)
{
    ...
    ChrClassesEntry const* classEntry = sDBCMgr->GetChrClassesEntry(createInfo.Class);
    if (!classEntry)
    {
    ...
    ChrRacesEntry const* raceEntry = sDBCMgr->GetChrRacesEntry(createInfo.Race);
    if (!raceEntry)
    {
    ...
    if (!AccountMgr::IsAdminAccount(GetSecurity()) && sObjectMgr->IsReservedName(createInfo.Name))
    {
        SendCharCreate(CHAR_NAME_RESERVED);
        return;
    }

    if (createInfo.Class == CLASS_DEATH_KNIGHT && !AccountMgr::IsAdminAccount(GetSecurity()))
    {
    ...
    */
    switch (_charCreateCallback.GetStage())
    {
        ...
        case 2:
        {
            ...
            if (skipCinematics == 1 || createInfo->Class == CLASS_DEATH_KNIGHT)
            {
        ...
        case 3:
        {
            ...
            uint32 skipCinematics = sWorld->getIntConfig(CONFIG_SKIP_CINEMATICS);
            bool checkHeroicReqs = createInfo->Class == CLASS_DEATH_KNIGHT && !AccountMgr::IsAdminAccount(GetSecurity());
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    if (pCurrChar->GetTeam() != pCurrChar->GetTeam(true) || pCurrChar->InBattleground())
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recvData)
{
    ...
    // check name limitations
    if (!AccountMgr::IsAdminAccount(GetSecurity()) && sObjectMgr->IsReservedName(renameInfo.Name))
    {
...
void WorldSession::HandleCharCustomize(WorldPacket& recvData)
{
    ...
    // check name limitations
    if (!AccountMgr::IsAdminAccount(GetSecurity()) && sObjectMgr->IsReservedName(customizeInfo.Name))
    {
...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recvData)
{
    ...
    // check name limitations
    if (!AccountMgr::IsAdminAccount(GetSecurity()) && sObjectMgr->IsReservedName(factionChangeInfo.Name))
    {
        ...
        if (recvData.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
        {
            ...
            for (std::map<uint32, uint32>::const_iterator it = sObjectMgr->FactionChangeReputation.begin(); it != sObjectMgr->FactionChangeReputation.end(); ++it)
            {
                ...
                if (PreparedQueryResult reputationResult = CharacterDatabase.Query(stmt))
                {
                    ...
                    // new base reputation
                    int32 newBaseRep = sObjectMgr->GetBaseReputationOf(sDBCMgr->GetFactionEntry(newReputation), factionChangeInfo.Race, playerClass);
