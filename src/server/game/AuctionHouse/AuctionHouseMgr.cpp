    ...
    for (AuctionEntryMap::const_iterator itr = AuctionsMap.begin(); itr != AuctionsMap.end(); ++itr)
    {
        ...
        if (!wsearchedname.empty())
        {
            ...
            if (propRefID)
            {
                ...
                if (propRefID < 0)
                {
                    const ItemRandomSuffixEntry* itemRandEntry = sDBCMgr->GetItemRandomSuffixEntry(-item->GetItemRandomPropertyId());
                    if (itemRandEntry)
                        suffix = itemRandEntry->nameSuffix;
                }
                else
                {
                    const ItemRandomPropertiesEntry* itemRandEntry = sDBCMgr->GetItemRandomPropertiesEntry(item->GetItemRandomPropertyId());
                    if (itemRandEntry)
                        suffix = itemRandEntry->nameSuffix;
                }
