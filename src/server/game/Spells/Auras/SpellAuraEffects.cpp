        ...
        if (ObjectGuid itemGUID = GetBase()->GetCastItemGUID())
            if (Player* playerCaster = caster->ToPlayer())
                if (Item* castItem = playerCaster->GetItemByGuid(itemGUID))
                    ...
                    {
                        ItemRandomSuffixEntry const* item_rand_suffix = sDBCMgr->GetItemRandomSuffixEntry(abs(castItem->GetItemRandomPropertyId()));
                        if (item_rand_suffix)
                        {
                            for (int k = 0; k < MAX_ITEM_ENCHANTMENT_EFFECTS; k++)
                            {
                                SpellItemEnchantmentEntry const* pEnchant = sDBCMgr->GetSpellItemEnchantmentEntry(item_rand_suffix->enchant_id[k]);
                                if (pEnchant)
                                {
    ...
    if (apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            for (uint32 i = 0; i < MAX_GLYPH_SLOT_INDEX; ++i)
            {
                ...
                {
                    if (GlyphPropertiesEntry const* glyph = sDBCMgr->GetGlyphPropertiesEntry(glyphId))
                    {
...
void AuraEffect::HandleAuraModShapeshift(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        SpellShapeshiftEntry const* shapeInfo = sDBCMgr->GetSpellShapeshiftEntry(form);
        ASSERT(shapeInfo);
...
void AuraEffect::HandleAuraOverrideSpells(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (OverrideSpellDataEntry const* overrideSpells = sDBCMgr->GetOverrideSpellDataEntry(overrideId))
...
void AuraEffect::HandleProcTriggerSpellAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        ...
        TC_LOG_DEBUG("spells","AuraEffect::HandleProcTriggerSpellAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in `spells` table.", triggerSpellId, GetId());
}
...
void AuraEffect::HandleProcTriggerSpellWithValueAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    if (SpellInfo const* triggeredSpellInfo = sSpellMgr->GetSpellInfo(triggerSpellId))
    {
        ...
        TC_LOG_DEBUG("spells","AuraEffect::HandleProcTriggerSpellWithValueAuraProc: Could not trigger spell %u from aura %u proc, because the spell does not have an entry in `spells` table.", triggerSpellId, GetId());
}
