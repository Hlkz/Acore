...
uint32 SpellMgr::GetSpellIdForDifficulty(uint32 spellId, Unit const* caster) const
{
    ...
    SpellDifficultyEntry const* difficultyEntry = sDBCMgr->GetSpellDifficultyEntry(difficultyId);
    if (!difficultyEntry)
    {
    ...
    {
        TC_LOG_ERROR("sql.sql", "SpellMgr::GetSpellIdForDifficulty: spell %u mode %u spell is 0. Check `spelldifficultydbc`!", spellId, mode);
        return spellId;
    }
...
SpellInfo const* SpellMgr::GetSpellForDifficultyFromSpell(SpellInfo const* spell, Unit const* caster) const
{
    ...
    if (!newSpell)
    {
        TC_LOG_DEBUG("spells", "SpellMgr::GetSpellForDifficultyFromSpell: spell %u not found. Check `spelldifficultydbc`!", newSpellId);
        return spell;
    }
...
void SpellMgr::LoadSpellTalentRanks()
{
    ...
    for (TalentContainer::const_iterator itr = sDBCMgr->TalentStore.begin(); itr != sDBCMgr->TalentStore.end(); ++itr)
    {
        ...
        if (!firstSpell)
        {
            TC_LOG_ERROR("spells", "SpellMgr::LoadSpellTalentRanks: First Rank Spell %u for TalentEntry %u does not exist.", talentInfo->RankID[0], itr->first);
            continue;
        }
        ...
        for (uint8 rank = 0; rank < MAX_TALENT_RANK; ++rank)
        {
            ...
            if (!currentSpell)
            {
                TC_LOG_ERROR("spells", "SpellMgr::LoadSpellTalentRanks: Spell %u (Rank: %u) for TalentEntry %u does not exist.", spellId, rank + 1, itr->first);
                break;
            }
        ...
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            if (entry->Effects[i].Effect == SPELL_EFFECT_LEARN_SPELL)
            {
                ...
                for (SpellLearnSpellMap::const_iterator itr = db_node_bounds.first; itr != db_node_bounds.second; ++itr)
                {
                    ...
                    {
                        TC_LOG_ERROR("sql.sql", "Spell %u auto-learn spell %u in `spells` table then the record in `spell_learn_spell` is redundant, please fix DB.",
...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    do
    {
        ...
        MapEntry const* mapEntry = sDBCMgr->GetMapEntry(st.target_mapId);
        if (!mapEntry)
        {
