...
m_bgGroup(NULL), m_baoGroup(NULL), m_lootMethod(FREE_FOR_ALL), m_lootThreshold(ITEM_QUALITY_UNCOMMON), m_looterGuid(),
...
bool Group::AddInvite(Player* player)
{
    ...
    if (group && (group->isBGGroup() || group->isBAOGroup()))
...
bool Group::RemoveMember(ObjectGuid guid, const RemoveMethod& method /*= GROUP_REMOVEMETHOD_DEFAULT*/, ObjectGuid kicker /*= 0*/, const char* reason /*= NULL*/)
{
    ...
    if (GetMembersCount() > ((isBGGroup() || isLFGGroup() || isBAOGroup()) ? 1u : 2u))
    {
...
void Group::SendUpdateToPlayer(ObjectGuid playerGUID, MemberSlot* slot)
{
    ...
    for (member_citerator citr = m_memberSlots.begin(); citr != m_memberSlots.end(); ++citr)
    {
        ...
        onlineState = onlineState | ((isBGGroup() || isBAOGroup()) ? MEMBER_STATUS_PVP : 0);
...
{
    if (player && !player->IsGameMaster() && sDBCMgr->GetMapEntry(player->GetMapId())->IsDungeon())
