...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
    ...
    switch (cond->SourceType)
    {
        ...
        case CONDITION_SOURCE_TYPE_SPELL_IMPLICIT_TARGET:
        {
            ...
            if (!spellInfo)
            {
                TC_LOG_ERROR("sql.sql", "%s in `condition` table, SourceEntry does not exist in `spells` table, ignoring.", cond->ToString().c_str());
                return false;
            }
        ...
        case CONDITION_SOURCE_TYPE_SPELL_PROC:
        {
            ...
            if (!spellProto)
            {
                TC_LOG_ERROR("sql.sql", "%s SourceEntry in `condition` table, does not exist in `spells` table, ignoring.", cond->ToString().c_str());
                return false;
            }
            break;
        }
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_SKILL:
        {
            SkillLineEntry const* pSkill = sDBCMgr->GetSkillLineEntry(cond->ConditionValue1);
            if (!pSkill)
            {
