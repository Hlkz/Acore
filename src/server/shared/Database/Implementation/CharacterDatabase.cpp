...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_GUID_TEAM_ACC_BY_NAME, "SELECT guid, team, account FROM characters WHERE name = ?", CONNECTION_BOTH);
	PrepareStatement(CHAR_SEL_GUID_RACE_ACC_BY_NAME, "SELECT guid, race, account FROM characters WHERE name = ?", CONNECTION_BOTH);
    PrepareStatement(CHAR_SEL_CHAR_TEAM, "SELECT team FROM characters WHERE guid = ?", CONNECTION_SYNCH);
    ...
    // Player saving
    PrepareStatement(CHAR_INS_CHARACTER, "INSERT INTO characters (guid, account, name, team, race, class, gender, level, xp, money, playerBytes, playerBytes2, playerFlags, "
    ...
    PrepareStatement(CHAR_UPD_CHAR_TEAM, "UPDATE characters SET team = ? WHERE guid = ?", CONNECTION_ASYNC);
	...
	//pvp ranks
	PrepareStatement(CHAR_SEL_PVP_RANK, "SELECT pvprank FROM character_rank WHERE guid = ?", CONNECTION_SYNCH);
	PrepareStatement(CHAR_UPD_PVP_RANK, "UPDATE character_rank SET pvprank = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_UPD_PVP_TOTAL, "UPDATE character_rank SET pvptotal = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_PVP_LAST, "SELECT pvplast FROM character_rank WHERE guid = ?", CONNECTION_SYNCH);
	PrepareStatement(CHAR_UPD_PVP_LAST, "UPDATE character_rank SET pvplast = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_BG_WIN, "SELECT bgwin FROM character_rank WHERE guid = ?", CONNECTION_SYNCH);
	PrepareStatement(CHAR_UPD_BG_WIN, "UPDATE character_rank SET bgwin = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_ARENA_WIN, "SELECT arenawin FROM character_rank WHERE guid = ?", CONNECTION_SYNCH);
	PrepareStatement(CHAR_UPD_ARENA_WIN, "UPDATE character_rank SET arenawin = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_UPD_PVP_POINTS, "UPDATE character_rank SET pvptotal = pvptotal + pvplast, pvplast = 0, pvprank = pvprank + ?, pvppoints = ? WHERE guid = ?", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_PLA_BY_RANK, "SELECT guid FROM character_rank WHERE pvprank <= ? AND pvprank >= ? ORDER BY pvppoints DESC", CONNECTION_ASYNC);
	PrepareStatement(CHAR_UPD_RESET_WIN_DAY, "UPDATE character_rank SET arenawin = 0, bgwin = 0 WHERE guid > 0", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_PVP_REC, "SELECT guid FROM character_rank WHERE guid = ?", CONNECTION_SYNCH);
	PrepareStatement(CHAR_INS_PVP_REC, "INSERT INTO character_rank (guid, pvprank, pvptotal, pvplast, arenawin, bgwin, pvppoints) VALUES (?, 0, 0, 0, 0, 0, 0)", CONNECTION_ASYNC);
	PrepareStatement(CHAR_SEL_PVP_ALL, "SELECT guid, pvprank, pvptotal, pvplast FROM character_rank WHERE guid > 0", CONNECTION_SYNCH);
