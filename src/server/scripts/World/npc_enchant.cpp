...
public:
    ...
    void Ench(Player *player, Creature *creature, uint32 slotid, uint32 enchid, uint32 twoha, uint32 requil, uint32 slty)
    {
        ...
        if(!item) {
            creature->Whisper("Vous devez vous équiper de l'objet.", LANG_UNIVERSAL, player);
            OnGossipHello(player, creature);
            return; }
        if (requil == 1 && item->GetTemplate()->ItemLevel <= 34) {
            creature->Whisper("L'objet n'est pas d'un niveau suffisant.", LANG_UNIVERSAL, player);
            OnGossipHello(player, creature);
            return; }
        if (twoha == 1 && item->GetTemplate()->InventoryType != 17) {
            creature->Whisper("Cette enchantement requiert une arme à deux mains.", LANG_UNIVERSAL, player);
            OnGossipHello(player, creature);
            return; }
        if (slotid == 16) {
        if ((item->GetTemplate()->Class == 4) && (item->GetTemplate()->SubClass != 6)) {
            creature->Whisper("Je n'enchante pas ce genre d'objets.", LANG_UNIVERSAL, player);
            player->CLOSE_GOSSIP_MENU();
            return; }
        if(item->GetTemplate()->SubClass == 6 && slty == 1) {
            creature->Whisper("Un enchantement d'arme ne va pas sur un bouclier.", LANG_UNIVERSAL, player);
            player->CLOSE_GOSSIP_MENU();
            return; }
        if(item->GetTemplate()->SubClass != 6 && slty == 0) {
            creature->Whisper("Un enchantement de bouclier ne va pas sur ce type d'arme.", LANG_UNIVERSAL, player);
            player->CLOSE_GOSSIP_MENU();
            return; } }
        SpellItemEnchantmentEntry const* enchant_id = sDBCMgr->GetSpellItemEnchantmentEntry(enchid);
        if (!enchant_id) {
            creature->Whisper("L'enchantement sélectionné n'est pas un enchantement, prévenez un administrateur.", LANG_UNIVERSAL, player);
            player->CLOSE_GOSSIP_MENU();
            return; }
