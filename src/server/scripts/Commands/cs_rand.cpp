...
bool AddItemChoix(Player *player, uint32 item_id, int choix) {
    ...
    if (dest.empty()) {
        ...
        return true; }
		if (Item* item = player->StoreNewItem(dest, item_id, true, Choix(item_id, choix)))
		{
...
static bool HandleComRand(ChatHandler* handler, const char* args) {
	...
	if (AddItemChoix(handler->GetSession()->GetPlayer(), item_id, ench_id))
		return true;
	return false; }

ChatCommand* GetCommands() const {
	static ChatCommand ComRand[] = {
		{ "rand",          SEC_GAMEMASTER,	    false, &HandleComRand,		"", NULL },
